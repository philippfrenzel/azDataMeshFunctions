...
def main(req: func.HttpRequest) -> func.HttpResponse:  # API version
    ...
    # If dataproduct claim not found, show block page. Dataproduct is required and sent by default.
    if 'dataproduct' not in req_body or not req_body.get('dataproduct') or "@" not in req_body.get('dataproduct'):
        return func.HttpResponse(
            json.dumps({"version": API_VERSION, "action": "ShowBlockPage", "userMessage": "Dataproduct is mandatory."}),
            status_code=200,
            mimetype="application/json"
        )
    ...
    if domain.lower() not in allowed_domains:
        ...
        return func.HttpResponse(
            json.dumps({"version": API_VERSION, "action": "ShowBlockPage",
                        "userMessage": f"You must have an account from '{s.join(allowed_domains)}' to register as an external user for Azure."}),
            status_code=200,
            mimetype="application/json"
        )
